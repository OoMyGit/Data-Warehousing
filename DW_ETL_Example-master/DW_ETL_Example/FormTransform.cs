    ...
    public partial class FormTransform : Form
    {
        ...
        private void btnTransform_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string cmdText = "SELECT \n    t.id_transaction,\n    t.id_user,\n    t.id_product,\n    t.total_price AS TotalAmount,\n    t.OrderOrigin\nFROM \n    transaction t\nJOIN \n    user u ON t.id_user = u.id_user\nJOIN \n    product p ON t.id_product = p.id_product;\n";
                myAdapter = new MySqlDataAdapter(cmdText, myConnOLAP);
                dt = new DataTable();
                myAdapter.Fill(dt);
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    myCmdOLAP.CommandText = "INSERT INTO fact (id_transaction, id_user, id_product, TotalAmount, OrderOrigin) VALUES " +
        ...
        private void showFactTable()
        {
            try
            {
                myAdapter = new MySqlDataAdapter("SELECT * from fact", myConnOLAP);
                dt = new DataTable();
                myAdapter.Fill(dt);
